...
export default class EventsManager {
	...
	loadEvents() {
		try {
			...
			return this._client.logger.warn(
				`[Modules] [${this._module.name}] [Events] No events found in module ${this._module.name}`
			);
		}
			...
			if (events && events.length > 0) {
				for (const event of events) {
					...
					if (cmdStats.isFile() && event.endsWith(".js")) {
						...
						const event = require(cmdPath)?.default;
						if (!event) continue;
						this.addEvent(new event(this._client));
					}
				}
			}
		}
