...
export default class Module {
    ...
    initModule() {
        try {
            ...
            this._client.logger.info(`[Modules] [${this.name}] [Commands] Loaded ${this.commandsManager.commands.size} commands`);

            this.eventsManager.loadEvents();
            this._client.logger.info(`[Modules] [${this.name}] [Events] Loaded ${this.eventsManager.events.size} events`);

            this.componentsManager.loadComponents();
            this._client.logger.info(`[Modules] [${this.name}] [Components] Loaded ${this.componentsManager.components.size} components`);
        } catch (error) {
            this._client.logger.error(`[Modules] [${this.name}] [Components] Error while loading components : ${error}`);
